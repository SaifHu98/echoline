...
# Note: AcceptDialog already has a 'confirmed' signal. We use a different
# signal name to avoid the override warning.
signal operation_confirmed(operation: String)
...
static func show_safe(parent: Node, operation: String, title: String, body: String,
...
func _build_buttons(warning_text: String, is_destructive: bool, on_confirm: Callable) -> void:
	# Add custom buttons in addition to defaults.
	...
	# Re-style the OK button that AcceptDialog auto-creates.
	...
	if _confirm_btn:
		...
		if not _confirm_btn.pressed.is_connected(_on_confirm):
...
func _on_confirm(on_confirm: Callable) -> void:
	...
	operation_confirmed.emit(_operation)
...
func _input(event: InputEvent) -> void:
	# Escape always cancels (no accidental confirmation).
